...
export function createPromoCodeValidation(): ReturnType<typeof body>[] {
  return [
    body('code').isString().trim().notEmpty().isLength({ min: 3, max: 30 }).withMessage('Code must be 3-30 characters'),
    ...
    body('minOrderAmount').optional().isInt({ min: 0 }).withMessage('Must be non-negative integer (cents)'),
    body('maxDiscountAmount').optional().isInt({ min: 1 }).withMessage('Must be positive integer (cents)'),
