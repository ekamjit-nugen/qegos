...
export interface OrderServiceResult {
  ...
  assignOrder: (id: string, staffId: string) => Promise<IOrderDocument2>;
  bulkAssign: (orderIds: string[], staffId: string) => Promise<{ updated: number }>;
  scheduleAppointment: (orderId: string, data: { date: string; timeSlot: string; type: string; staffId: string; meetingLink?: string }) => Promise<IOrderDocument2>;
...
export function createOrderService(deps: OrderServiceDeps): OrderServiceResult {
  const { OrderModel, SalesModel } = deps;
  ...
  async function assignOrder(id: string, staffId: string): Promise<IOrderDocument2> {
