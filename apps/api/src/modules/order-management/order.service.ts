...
export interface OrderServiceResult {
  ...
  updateOrder: (id: string, data: Partial<IOrderDocument2>, scopeFilter?: Record<string, unknown>) => Promise<IOrderDocument2>;
  transitionStatus: (id: string, newStatus: number, data: { note?: string; eFileReference?: string; cancelReason?: string }, actorUserType: number, scopeFilter?: Record<string, unknown>) => Promise<IOrderDocument2>;
  assignOrder: (id: string, staffId: string, scopeFilter?: Record<string, unknown>) => Promise<IOrderDocument2>;
  bulkAssign: (orderIds: string[], staffId: string) => Promise<{ updated: number }>;
  scheduleAppointment: (orderId: string, data: { date: string; timeSlot: string; type: string; staffId: string; meetingLink?: string }, scopeFilter?: Record<string, unknown>) => Promise<IOrderDocument2>;
  updateProgress: (id: string, percent: number, scopeFilter?: Record<string, unknown>) => Promise<IOrderDocument2>;
  calculateTotals: (orderId: string) => Promise<{ totalAmount: number; discountAmount: number; finalAmount: number }>;
  getStats: () => Promise<Record<string, unknown>>;
  getRevenue: (filters?: { financialYear?: string; dateFrom?: string; dateTo?: string }) => Promise<Record<string, unknown>>;
...
export function createOrderService(deps: OrderServiceDeps): OrderServiceResult {
  ...
  async function listOrders(query: OrderListQuery): Promise<OrderListResult> {
    ...
    if (query.processingBy) filter.processingBy = query.processingBy;
    if (query.userId) filter.userId = query.userId;
    if (query.eFileStatus) filter.eFileStatus = query.eFileStatus;
  ...
  async function updateOrder(
    ...
    if (data.lineItems || data.discountPercent !== undefined) {
      const totalAmount = existing.lineItems.reduce(
  ...
  async function transitionStatus(
    ...
    if (newStatus === OrderStatus.Cancelled) {
      ...
      if (data.note) order.notes = (order.notes ? order.notes + '\n' : '') + `CANCELLED: ${data.cancelReason}`;
    }
    ...
    if (newStatus === OrderStatus.Completed) {
      if (!ReviewAssignmentModel) {
        throw AppError.badRequest('Review system not configured — cannot complete order (RVW-INV-01)');
      }
      ...
      if (!approvedReview) {
        throw AppError.badRequest('Order cannot be completed without an approved review (RVW-INV-01)');
      }
    }
    ...
    if (newStatus === OrderStatus.Lodged) {
      if (!data.eFileReference && !order.eFileReference) {
        throw AppError.badRequest('eFileReference is required for lodgement, or provide manual confirmation');
      }
