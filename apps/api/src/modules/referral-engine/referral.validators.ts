...
export function validateApply(): ValidationChain[] {
  return [
    body('referralCode')
      ...
      .matches(/^QGS-REF-\d{4,}$/i)
      .withMessage('Invalid referral code format'),
    body('refereeUserId')
      .trim()
      .notEmpty()
      .withMessage('Referee user ID is required')
      .isMongoId()
      .withMessage('Referee user ID must be a valid ID'),
    body('refereeLeadId')
      .optional()
      .trim()
      .isMongoId()
      .withMessage('Referee lead ID must be a valid ID'),
  ];
}

export function validateCode(): ValidationChain[] {
  return [param('code').trim().notEmpty().withMessage('Code is required')];
}
...
export function validateConfigUpdate(): ValidationChain[] {
  return [
    ...
    body('rewardType').optional().isIn(REFERRAL_REWARD_TYPES).withMessage('Invalid reward type'),
    body('referrerRewardValue')
      .optional()
      .isInt({ min: 0 })
      .withMessage('Referrer reward must be a non-negative integer (cents)')
      .toInt(),
    body('refereeRewardValue')
      .optional()
      .isInt({ min: 0 })
      .withMessage('Referee reward must be a non-negative integer (cents)')
      .toInt(),
    body('maxReferralsPerClient')
      .optional()
      .isInt({ min: 1 })
      .withMessage('Max referrals must be a positive integer')
      .toInt(),
    ...
    body('minimumOrderValueForReward')
      .optional()
      .isInt({ min: 0 })
      .withMessage('Minimum order value must be a non-negative integer (cents)')
      .toInt(),
  ];
}
