...
export function calculateTaxEstimate(
  ...
  if (input.capitalGains.longTerm > 0 && isResident) {
    const discountAmount = input.capitalGains.longTerm - Math.round(input.capitalGains.longTerm * (1 - rules.cgtDiscount));
    warnings.push(`CGT 50% discount applied: $${(discountAmount / 100).toFixed(2)} excluded from long-term gains`);
  }
  ...
  breakdown.push({ label: 'Employment Income', amount: input.grossEmploymentIncome, type: 'income' });
  ...
  if (input.dividendFrankingCredits > 0) {
    breakdown.push({ label: 'Franking Credits', amount: input.dividendFrankingCredits, type: 'income' });
  }
  if (capitalGainsCalculated > 0) {
    breakdown.push({ label: 'Capital Gains (after discount)', amount: capitalGainsCalculated, type: 'income' });
  }
  ...
  if (input.governmentPayments > 0) {
    breakdown.push({ label: 'Government Payments', amount: input.governmentPayments, type: 'income' });
  }
  if (input.superannuationIncome > 0) {
    breakdown.push({ label: 'Superannuation Income', amount: input.superannuationIncome, type: 'income' });
  }
  ...
  let totalDeductions =
    d.workRelated + d.selfEducation + d.vehicleExpenses + d.homeOffice +
    d.donations + d.incomeProtection + d.accountingFees + d.other;

  // Standard deduction shortcut: $300 work-related only
  const nonWorkDeductions = d.selfEducation + d.vehicleExpenses + d.homeOffice +
    d.donations + d.incomeProtection + d.accountingFees + d.other;
  ...
  if (isResident) {
    ...
    const effectiveThreshold = (input.isEligibleSenior && seniorThreshold > 0)
      ? seniorThreshold
      : ml.lowIncomeThreshold;
    ...
    if (taxableIncome <= effectiveThreshold) {
      ...
      medicareLevyAmount = Math.round((taxableIncome - effectiveThreshold) * 0.10);
    } else {
    ...
    if (input.spouseIncome !== undefined || input.numberOfDependants > 0) {
      const familyThreshold = ml.familyThreshold +
        (input.numberOfDependants * ml.additionalChildAmount);
      ...
      if (combinedIncome <= familyThreshold) {
        ...
        const familyLevy = Math.round((combinedIncome - familyThreshold) * 0.10);
  ...
  if (medicareLevySurcharge > 0) {
    breakdown.push({ label: 'Medicare Levy Surcharge', amount: medicareLevySurcharge, type: 'levy' });
  }
  ...
  if (isResident) {
    ...
    if (taxableIncome <= lito.lowerThreshold) {
      ...
      litoOffset = lito.maxOffset - Math.round((taxableIncome - lito.lowerThreshold) * lito.reductionRate);
  ...
  if (isResident && input.isEligibleSenior && rules.sapto) {
    ...
    if (taxableIncome <= sapto.thresholdSingle) {
      ...
      saptoOffset = sapto.maxSingle - Math.round((taxableIncome - sapto.thresholdSingle) * sapto.phaseOutRate);
      saptoOffset = Math.max(0, saptoOffset);
    }
  }

  if (saptoOffset > 0) {
    breakdown.push({ label: 'Senior Australians Tax Offset (SAPTO)', amount: saptoOffset, type: 'offset' });
  }
  ...
  if (excessCredits > 0) {
    ...
    warnings.push(
      `Excess franking credits of $${(excessCredits / 100).toFixed(2)} refunded`,
    );
  }
  ...
  const effectiveTaxRate = grossIncome > 0
    ? Math.round((totalTaxPayable / grossIncome) * 10000) / 100
    : 0;
