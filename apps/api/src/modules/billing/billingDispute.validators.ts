...
export function validateCreateDispute(): ValidationChain[] {
  return [
    ...
    body('disputedAmount')
      .notEmpty()
      .withMessage('Disputed amount is required')
      .isInt({ min: 1 })
      .withMessage('Disputed amount must be a positive integer (cents)')
      .toInt(),
    body('clientStatement')
      .trim()
      .notEmpty()
      .withMessage('Client statement is required')
      .isLength({ max: 2000 })
      .withMessage('Client statement must be at most 2000 characters'),
    body('ticketId').optional().trim().isMongoId().withMessage('Ticket ID must be a valid ID'),
  ];
}
...
export function validateUpdateDispute(): ValidationChain[] {
  return [
    ...
    body('resolvedAmount')
      .optional()
      .isInt({ min: 0 })
      .withMessage('Resolved amount must be a non-negative integer (cents)')
      .toInt(),
  ];
}

export function validateDisputeId(): ValidationChain[] {
  return [param('id').trim().notEmpty().isMongoId().withMessage('Dispute ID must be a valid ID')];
}
