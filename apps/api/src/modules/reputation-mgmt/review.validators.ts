...
export function validateSubmitReview(): ValidationChain[] {
  return [
    ...
    body('tags.*').optional().isIn(REVIEW_TAGS).withMessage('Invalid review tag'),
  ];
}
...
export function validateRespondReview(): ValidationChain[] {
  return [
    ...
    body('response')
      ...
      .isLength({ max: 2000 })
      .withMessage('Response must be at most 2000 characters'),
  ];
}

export function validateReviewId(): ValidationChain[] {
  return [param('id').trim().notEmpty().isMongoId().withMessage('Review ID must be a valid ID')];
}
