...
export function createAppointmentValidation(): ReturnType<typeof body>[] {
  return [
    body('userId')
      ...
      .isMongoId().withMessage('userId must be a valid ObjectId'),
    body('staffId')
      .notEmpty().withMessage('staffId is required')
      .isMongoId().withMessage('staffId must be a valid ObjectId'),
    body('date')
      .notEmpty().withMessage('date is required')
      .isISO8601().withMessage('date must be a valid ISO 8601 date'),
    body('startTime')
      .notEmpty().withMessage('startTime is required')
      .matches(TIME_PATTERN).withMessage('startTime must be HH:mm format'),
    body('endTime')
      .notEmpty().withMessage('endTime is required')
      .matches(TIME_PATTERN).withMessage('endTime must be HH:mm format')
    ...
    body('type')
      ...
      .isIn([...APPOINTMENT_TYPES]).withMessage(`type must be one of: ${APPOINTMENT_TYPES.join(', ')}`),
    body('orderId')
      .optional()
      .isMongoId().withMessage('orderId must be a valid ObjectId'),
    body('meetingLink')
      .optional()
 ...
 */
export function updateAppointmentValidation(): Array<ReturnType<typeof body> | ReturnType<typeof param>> {
  return [
 ...
 */
export function statusTransitionValidation(): Array<ReturnType<typeof body> | ReturnType<typeof param>> {
  return [
    param('id').isMongoId().withMessage('Invalid appointment ID'),
    body('status')
 ...
 */
export function staffAvailabilityValidation(): Array<ReturnType<typeof body> | ReturnType<typeof param>> {
  return [
    param('staffId').isMongoId().withMessage('Invalid staff ID'),
    body('dayOfWeek')
      .isInt({ min: 0, max: 6 }).withMessage('dayOfWeek must be 0 (Sun) through 6 (Sat)'),
    body('startTime')
      .notEmpty().withMessage('startTime is required')
      .matches(TIME_PATTERN).withMessage('startTime must be HH:mm format'),
    body('endTime')
    ...
    body('blockDate')
      .optional()
      .isISO8601().withMessage('blockDate must be a valid ISO 8601 date'),
    body('blockReason')
      .optional()
      .trim(),
  ];
 ...
 */
export function availabilityQueryValidation(): Array<ReturnType<typeof query> | ReturnType<typeof param>> {
  return [
    param('staffId').isMongoId().withMessage('Invalid staff ID'),
    query('dateFrom')
