...
export function createAppointmentRoutes(
...
): { appointmentRouter: Router; staffAvailabilityRouter: Router } {
  ...
  appointmentRouter.get('/', ...auth, ...listAppointmentValidation(), async (req: Request, res: Response) => {
    if (handleValidation(req, res)) return;
    try {
  ...
  appointmentRouter.get('/:id', ...auth, ...appointmentIdValidation(), async (req: Request, res: Response) => {
    if (handleValidation(req, res)) return;
    try {
      const appointment = await service.getAppointment(req.params.id);
      if (!appointment) {
        res.status(404).json({ status: 404, code: 'NOT_FOUND', message: 'Appointment not found' });
        return;
      }
  ...
  appointmentRouter.patch('/:id', ...authUpdate, ...updateAppointmentValidation(), async (req: Request, res: Response) => {
    if (handleValidation(req, res)) return;
    try {
      const updated = await service.updateAppointment(req.params.id, req.body as Record<string, unknown>);
      if (!updated) {
        res.status(404).json({ status: 404, code: 'NOT_FOUND', message: 'Appointment not found' });
        return;
      }
  ...
  appointmentRouter.patch('/:id/status', ...authUpdate, ...statusTransitionValidation(), async (req: Request, res: Response) => {
    if (handleValidation(req, res)) return;
    try {
  ...
  appointmentRouter.delete('/:id', ...authDelete, ...appointmentIdValidation(), async (req: Request, res: Response) => {
    if (handleValidation(req, res)) return;
    try {
