...
export function LeadDetailPage({ id }: { id: string }): React.ReactNode {
  ...
  const isCallType = ['phone_call_outbound', 'phone_call_inbound', 'phone_call_missed'].includes(
  ...
  if (!lead) {
    return <Empty description="Lead not found" />;
  }

  const activities = activitiesData?.data ?? [];
  const canConvert =
    !lead.isConverted && [LeadStatus.QuoteSent, LeadStatus.Negotiation].includes(lead.status);
  ...
  return (
    <div>
            ...
            <Space>
              {lead.convertedOrderId && (
                <Button
                  ...
                  size="small"
                  icon={<LinkOutlined />}
                  onClick={() => router.push(`/orders/${lead.convertedOrderId}`)}
                >
                  View Order
                </Button>
              )}
              {lead.convertedUserId && (
                <Button
                  ...
                  size="small"
                  icon={<LinkOutlined />}
                  onClick={() => router.push(`/users/${lead.convertedUserId}`)}
                >
                  View Client
                </Button>
              )}
      ...
      <Row gutter={[16, 16]}>
                  ...
                  children: (
                    <Descriptions column={{ xs: 1, sm: 2 }} size="small">
                      ...
                      <Descriptions.Item label="Preferred Language">
                        {PREFERRED_LANGUAGE_LABELS[lead.preferredLanguage ?? ''] ??
                          lead.preferredLanguage ??
                          '—'}
                      </Descriptions.Item>
                      <Descriptions.Item label="Preferred Contact">
                        {PREFERRED_CONTACT_LABELS[lead.preferredContact ?? ''] ??
                  ...
                  children: (
                    <Descriptions column={{ xs: 1, sm: 2 }} size="small">
                      ...
                      <Descriptions.Item label="Priority">
                        <Tag color={LEAD_PRIORITY_COLORS[lead.priority]}>
                  ...
                  children: (
                    <Descriptions column={{ xs: 1, sm: 2 }} size="small">
                      ...
                      <Descriptions.Item label="Employment">
                        {EMPLOYMENT_TYPE_LABELS[lead.employmentType ?? ''] ??
                          lead.employmentType ??
                          '—'}
                      </Descriptions.Item>
                      <Descriptions.Item label="Has Spouse">
                        {lead.hasSpouse != null ? (lead.hasSpouse ? 'Yes' : 'No') : '—'}
                      </Descriptions.Item>
                      <Descriptions.Item label="Dependants">
                        {lead.numberOfDependants ?? '—'}
                      </Descriptions.Item>
                      <Descriptions.Item label="Rental Property">
                        {lead.hasRentalProperty != null
                          ? lead.hasRentalProperty
                            ? 'Yes'
                            : 'No'
                          : '—'}
                      </Descriptions.Item>
                      <Descriptions.Item label="Share Portfolio">
                        {lead.hasSharePortfolio != null
                ...
                },
                {
                  ...
                  children: (
                    <Descriptions column={{ xs: 1, sm: 2 }} size="small">
                      ...
                      <Descriptions.Item label="Follow-up Count">
                        {lead.followUpCount ?? 0}
                      </Descriptions.Item>
                      <Descriptions.Item label="Next Action">
                        {lead.nextAction ?? '—'}
                      </Descriptions.Item>
                      <Descriptions.Item label="Next Action Date">
        ...
        <Col xs={24} lg={8}>
          ...
          >
            {lead.assignedToName ? (
              <p>
                Assigned to: <strong>{lead.assignedToName}</strong>
              </p>
            ) : (
          ...
          >
            {pendingReminders.length > 0 && (
              <>
                <Divider orientation="left" style={{ fontSize: 12, margin: '4px 0 8px' }}>
                  Pending
                </Divider>
                {pendingReminders.map((r) => (
                  <div
                    ...
                    style={{
                      marginBottom: 12,
                      padding: 8,
                      background: r.isOverdue ? '#fff2f0' : '#fafafa',
                      borderRadius: 6,
                    }}
                  >
                    ...
                    <Button
                      ...
                      type="link"
                      onClick={() => {
                        void completeReminder.mutateAsync(r._id);
                      }}
                    >
                      Complete
                    </Button>
          ...
          {lead.status === LeadStatus.Lost && lead.lostReason && (
            <Card title="Lost Reason" style={{ marginBottom: 16 }}>
              <Tag color="red">
                {lead.lostReason.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())}
              </Tag>
              {lead.lostReasonNote && (
                <p style={{ marginTop: 8, color: '#666' }}>{lead.lostReasonNote}</p>
              )}
            </Card>
          )}
        </Col>
              ...
              <Form.Item name="outcome" label="Outcome">
                <Select
                  ...
                  options={ACTIVITY_OUTCOMES.map((o) => ({
                    value: o,
                    label: ACTIVITY_OUTCOME_LABELS[o],
                  }))}
                />
              </Form.Item>
            </Col>
      ...
      <Modal
        ...
        footer={[
          <Button key="cancel" onClick={() => setShowConvertModal(false)}>
            Cancel
          </Button>,
          <Button
            key="convert"
      ...
      >
        ...
        <ul>
          ...
          <li>
            Mark the lead as <Tag color="green">Won</Tag>
          </li>
        </ul>
        <Alert
          type="info"
