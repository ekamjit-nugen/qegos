...
export const uploadDocumentValidation = [
  body('financialYear')
    ...
    .matches(fyRegex)
    .withMessage('financialYear must be YYYY-YY format'),
  body('category')
    .isString()
...
export const getDocumentValidation = [param('id').isMongoId().withMessage('Invalid document ID')];
...
export const listDocumentsValidation = [
  query('financialYear')
    ...
    .matches(fyRegex)
    .withMessage('financialYear must be YYYY-YY format'),
  query('category').optional().isIn(VAULT_DOCUMENT_CATEGORIES),
...
export const prefillValidation = [
  param('financialYear').matches(fyRegex).withMessage('financialYear must be YYYY-YY format'),
];
...
export const createTaxSummaryValidation = [
  ...
  body('totalIncome')
    .isInt({ min: 0 })
    .withMessage('totalIncome must be non-negative integer (cents)'),
  body('totalDeductions')
    .isInt({ min: 0 })
    .withMessage('totalDeductions must be non-negative integer (cents)'),
  body('taxableIncome')
    .isInt({ min: 0 })
    .withMessage('taxableIncome must be non-negative integer (cents)'),
...
export const yoyComparisonValidation = [
  param('year').matches(fyRegex).withMessage('year must be YYYY-YY format'),
];
...
export const updateAtoStatusValidation = [
  param('year').matches(fyRegex).withMessage('year must be YYYY-YY format'),
  body('userId').isMongoId().withMessage('Invalid userId'),
  body('atoRefundStatus').isIn(ATO_REFUND_STATUSES).withMessage('Invalid ATO refund status'),
  body('assessmentDate').optional().isISO8601(),
  body('noaReceived').optional().isBoolean(),
...
export const bulkUpdateAtoStatusValidation = [
  ...
  body('updates.*.financialYear')
    .matches(fyRegex)
    .withMessage('financialYear must be YYYY-YY format'),
  body('updates.*.atoRefundStatus').isIn(ATO_REFUND_STATUSES),
