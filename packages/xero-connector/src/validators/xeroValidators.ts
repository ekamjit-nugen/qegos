...
export function validateConfigUpdate(): ValidationChain[] {
  return [
    body('xeroRevenueAccountCode')
      .optional().trim().isLength({ max: 20 }).withMessage('Account code too long'),
    body('xeroBankAccountId')
      .optional().trim().isLength({ max: 100 }),
    body('xeroGstAccountCode')
      .optional().trim().isLength({ max: 20 }),
    body('xeroDefaultTaxType')
      .optional().trim().isLength({ max: 20 }),
  ];
}
...
export function validateSyncLogId(): ValidationChain[] {
  return [
    param('syncLogId')
      .trim().notEmpty().isMongoId().withMessage('Sync log ID must be a valid ID'),
  ];
}
...
export function validateSyncLogList(): ValidationChain[] {
  return [
    ...
    query('entityType')
      .optional().isIn(['contact', 'invoice', 'payment', 'credit_note']),
    query('status')
      .optional().isIn(['queued', 'processing', 'success', 'failed']),
...
export function validateCreditNote(): ValidationChain[] {
  return [
    ...
    body('refundAmountCents')
      ...
      .isInt({ min: 1 }).withMessage('Refund amount must be a positive integer (cents)')
      .toInt(),
    body('reference')
      .trim().notEmpty().withMessage('Reference is required')
      .isLength({ max: 200 }),
  ];
}
